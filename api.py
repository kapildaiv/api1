...
def configure_ngrok():
    ngrok_token = "2pKvTMiHqa5HreSXbtoI5Dy9LEu_6jK1hy7hbuf2rZuzgt3AM"
    try:
...
def update_soul_txt(public_url):
    with open("sid1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in sid1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("sid1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated sid1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update sid1.txt on VPS: {str(e)}")
